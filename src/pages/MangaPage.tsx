import { type ReactNode, useCallback, useEffect, useMemo, useRef, useState, memo, type CSSProperties } from "react";
import { createPortal } from "react-dom";
import { BookOpen, BookText, ChevronLeft, ChevronRight, Clock, Compass, ExternalLink, Inbox, RefreshCw, ShoppingBag, Star, Tags, Users, X } from "lucide-react";
...
const ALL_MODES: MangaMode[] = ["latest", "followed", "rating"];
const mangaCacheKey = (m: MangaMode) => `mangaPage:v2:${m}`;
...
/* 10.1/10.2/10.3: overlay 漫画卡片——imageState 可恢复错误、decoding=async、CSS 变量延迟 */
const MangaCard = memo(function MangaCard({ manga, index, priority, onOpen }: {
  manga: MangaUpdateItem;
  index: number;
  priority?: boolean;
  onOpen: () => void;
}) {
  ...
  return (
    <article
      ...
      className="anime-card-enter text-left acg-image-card rounded-2xl liquid-panel overflow-hidden group cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70 transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5"
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-slate-200 dark:bg-slate-700">
        ...
        {/* 加载失败占位——保留封面区域示意 */}
        {(!coverUrl || imageState === "error") && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-1.5 p-2 image-error bg-emerald-100/60 dark:bg-emerald-900/20">
            <BookOpen className="w-6 h-6 text-emerald-400" />
            <span className="text-xs text-emerald-600 dark:text-emerald-300 text-center line-clamp-2 leading-tight">{manga.title}</span>
          </div>
        )}

        {/* overlay 信息浮层：仅在图片呈现时叠加 */}
        {imageState !== "error" && coverUrl && (
          <>
            <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/30 to-transparent" />
            {/* 顶部徽标行 */}
            <div className="absolute top-2 left-2 right-2 z-10 flex items-start justify-between gap-1 flex-wrap">
              {index < 3 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500 text-white font-semibold shadow">TOP {index + 1}</span>
              )}
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium shadow-sm ${st.color}`}>{st.label}</span>
              {manga.latestChapter && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-violet-500/80 text-white font-medium ml-auto">第{manga.latestChapter}话</span>
              )}
            </div>
            {/* 底部标题区 */}
            <div className="absolute bottom-0 left-0 right-0 p-3 pt-8">
              <h3 className="text-sm font-bold text-white leading-snug line-clamp-2 drop-shadow-md">{manga.title}</h3>
              <span className="flex items-center gap-1 text-[10px] text-white/70 mt-1">
                <Clock className="w-3 h-3" /> {timeAgo(manga.updatedAt)}
              </span>
              {/* tags 压缩行 */}
              {manga.tags.length > 0 && (
...
export default function MangaPage() {
  ...
  const changeSelection = useCallback((next: number) => {
    setSelectedIndex(next);
    setSelectedManga(items[next] ?? null);
  }, [items]);
  ...
  /* 跨 mode 后台预缓存（与 P 站同模式）：活跃 mode 加载完成后串行预取其余 mode，
  ...
  useEffect(() => {
    ...
    const arm = () => {
      ...
      stop = prefetchAcgModes<MangaUpdateItem[]>(
        mangaCacheKey,
        others,
        async (m) => {
          const res = await fetch(`/api/public/manga-updates?mode=${m}`, { cache: "force-cache" });
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
          const json = await res.json();
          if (json.code !== 0 || !json.data) throw new Error(json.error || "API error");
          return json.data as MangaUpdateItem[];
        },
        PREFETCH_INTERVAL_MS,
      );
    };

    const curSwr = readAcgPageCacheSWR<MangaUpdateItem[]>(mangaCacheKey(mode), CACHE_TTL_MS);
    if (curSwr) arm();
    else {
      const t = setInterval(() => {
        const swr = readAcgPageCacheSWR<MangaUpdateItem[]>(mangaCacheKey(mode), Infinity);
        if (swr || cancelled) {
          clearInterval(t);
          if (!cancelled) arm();
        }
  ...
  return (
    <CommunityGate unlocked={communityUnlocked} checking={communityChecking} unlock={unlockCommunity} title="漫画情报">
              ...
              {colItems.map((item, itemIdx) => {
                ...
                return (
                  <MangaCard
                    ...
                    onOpen={() => { setSelectedIndex(originalIndex); setSelectedManga(item); }}
                  />
                );
              })}
            </div>
...
function MangaDetailModal({
  ...
  /* 相邻作品封面预读：切换 index 时按方向预读本方向相邻 2 部封面，命中后切换不闪烁。 */
  ...
  useEffect(() => {
    ...
    if (dir === 0) return;
    const targets = [index + dir, index + 2 * dir].filter((i) => i >= 0 && i < total);
    if (targets.length === 0) return;
    const schedule = () => {
      for (const i of targets) {
        const it = allItems[i];
        if (!it || !it.coverUrl) continue;
        const img = document.createElement("img");
        img.src = toMangaCoverUrl(it.coverUrl);
      }
    };
  ...
  }, [onClose, index, total, onPrev, onNext]);

  return createPortal(
    <div role="dialog" aria-label="漫画详情" className="liquid-overlay fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6">
        ...
        <div className="shrink-0 flex items-center justify-between gap-4 px-4 sm:px-6 py-4 border-b border-white/30 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
          <div className="flex items-center gap-1.5 min-w-0">
            <button
              ...
              onClick={onPrev}
              disabled={index <= 0}
              className="p-1.5 rounded-full text-slate-400 hover:text-emerald-500 hover:bg-white/40 dark:hover:bg-slate-800/50 disabled:opacity-30 disabled:hover:bg-transparent transition-colors shrink-0"
              aria-label="上一部漫画"
            >
            ...
            <h2 className="ml-1 text-base sm:text-lg font-bold text-slate-800 dark:text-white truncate">{manga.title}</h2>
          </div>
